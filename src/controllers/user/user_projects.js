...
import { onSuccess, onError, returnNumber} from "../../utils";
import {
  calculateSimilarProjectEquipment,
  calculateSimilarTemplateEquipment,
  calculateSimilarProjectMaterial,
  calculateSimilarTemplateMaterial,
  calculateSimilarProjectLabour,
...
class UserProjectsController {
  // get all projects
  static async getAllProjectsTemplates(req, res) {
    try {
      ...
      return onSuccess(res, 200, "Success", template);
    } catch (error) {
      return onError(
        res,
        500,
  ...
  static async getProjectByStatus(req, res) {
    try {
      const { id: userId, role } = req.user;
      const { status } = req.query;
      if (role === "admin") {
        const projects = await Projects.findAll({
          where: { adminId: userId, status: status },
          order: [["createdAt", "DESC"]],
        });
        return onSuccess(res, 200, "Projects Fetched Successfully", projects);
      }
  ...
  static async getProjectById(req, res) {
    try {
      ...
      if (role === "owner") {
        ...
        return onSuccess(res, 200, "All template returned Successfully", templateData);
      }
    } catch (error) {
  ...
  static async createProject(req, res) {
    try {
      const { id: userId, managerId, role } = req.user;
      const { name, vat, groupId } = req.body;

      // convert vat to percentage
      const vatPercentage = vat <= 0 ? 0 : vat * 0.01;
      ...
      if (role === "owner") {
        // project name already exists
        const templateExist = await Templates.findOne({
          where: { name },
        });
        if (templateExist) {
          return onError(res, 400, "Template name already exists");
        }
        // create project
        ...
        return onSuccess(res, 201, "Created Successfully", template);
      }
    } catch (error) {
      console.log(error);
      return onError(
  ...
  static async editProject(req, res) {
    try {
      ...
      if (role === "manager" || role === "admin") {
        ...
        // get editedvalue
        const editedName = name ? name : project.name;
        const editedDescription = description
          ? description
          : project?.description;
        const editedClient = client ? client : project?.client;
        const editedContractor = contractor ? contractor : project?.contractor;
        const editedConsultant = consultant ? consultant : project?.consultant;
        const editedStartDate = startDate ? startDate : project?.startDate;
        const editedEndDate = endDate ? endDate : project?.endDate;
        const editedStatus = status ? status : project?.status;
        const existingCurrencyCode = project?.currencyCode;

        const editedVAT = vat ? vat * 0.01 : +project?.vat;
        const approved = isApproved ? isApproved : project.isApproved;
        ...
        for (let estimation of estimations) {
          ...
          const labours = estimation?.labours;
          const subContractors = estimation?.subContractors;
          const estimationQty = +estimation?.estimationQuantity;
          const wastage = +estimation?.wastagePercentage;
          const transport = +estimation?.transportPercentage;
          const profit = +estimation?.profitPercentage;
          const overHead = +estimation?.overHeadPercentage;
          const contigency = +estimation?.contigencyPercentage;

          const oldEstimationRate = +estimation?.estimationRate;
          const oldEstimationTotalAmount = +estimation?.estimationTotalAmount;
          ...
          const hasEquipment = equipments.length === 0 ? true : false;
          const hasMaterial = materials.length === 0 ? true : false;
          const hasLabour = labours.length === 0 ? true : false;
          const hasSubcontractor = subContractors.length === 0 ? true : false;
          const rate = hasEquipment && hasMaterial && hasLabour && hasSubcontractor ? true : false;
          ...
          for (let i = 0; i < equipments.length; i++) {
            ...
            // apply editedCurrencyValue
            const appliedPrice = +equipment.hireRatePrice;
            let currencyConverter;
            if (appliedPrice > 0) {
              currencyConverter = new CC({
                from: existingCurrencyCode,
                to: currencyCode,
                amount: appliedPrice,
              });
            }
            ...
            const finalPrice = response;

            const calculatedPrice =
              +equipment?.number * finalPrice * equipmentPerformance;
            ...
            // update equipment
            const equip = await UserEquipments.findOne({
              where: {
                id: equipment.id,
              },
            });
            await equip.update({
              hireRatePrice: finalPrice,
            });
          ...
          for (let i = 0; i < materials.length; i++) {
            ...
            const appliedPrice = +material.price;
            let currencyConverter;
            if (appliedPrice > 0) {
              currencyConverter = new CC({
                from: existingCurrencyCode,
            ...
            const materialTotalAmount = materialFactorQuantity * finalPrice;

            // update material
            const mat = await UserMaterials.findOne({
              where: {
                id: material.id,
              },
            });
          ...
          for (let i = 0; i < labours.length; i++) {
            ...
            const appliedPrice = +labor.wages;
          ...
          for (let i = 0; i < subContractors.length; i++) {
            ...
            const response = await currencyConverter?.convert();
            const finalPrice = response;
            const subContractorTotalAmount = finalPrice;

            // update subcontractor
            const sub = await UserSubContractors.findOne({
              where: {
                id: subc.id,
              },
            });
            ...
            const updatesub = await UserSubContractors.findOne({
              where: { id: subc.id },
            });
            await updatesub.update({ quantity: estimationQty });
            // Update estimationLibrary
            const estimationLibary = await UserEstimationLibrary.findOne({
              where: {
                estimationId: estimationId,
          ...
          const equipmentRatePerUnit =
            equipmentCostPerWorkiItem / estimationQty;
          // labour
          ...
          const calculatedOverHead =
            newEstimationRate * estimationQty * overHead;
          const tempProfit =
            newEstimationRate * estimationQty + calculatedOverHead;
          const calculatedProfit = tempProfit * profit;
          const tempContigency =
            newEstimationRate * estimationQty + calculatedProfit;
          ...
          await estimation.update({
            ...
            estimationRate: rate ? oldEstimationRate : ratePerUnit?.toFixed(2),
            estimationTotalAmount: rate ? oldEstimationTotalAmount : totalAmount?.toFixed(2),
      ...
      if (role === "owner") {
        ...
        if (!template) {
          return onError(res, 404, "Template Not Found");
        }

        // get editedvalue
        const editedName = name ? name : template.name;
        const editedDescription = description
          ? description
          : template?.description;
        const editedStatus = status ? status : template?.status;

        const editedVAT = vat ? vat * 0.01 : +template?.vat;
        const approved = isApproved ? isApproved : template.isApproved;
        const existingCurrencyCode = template?.currencyCode;
        ...
        for (let estimation of estimations) {
          ...
          for (let i = 0; i < subContractors.length; i++) {
            ...
            const updatesub = await SubContractors.findOne({
              where: { id: subc.id },
            });
            await updatesub.update({ quantity: estimationQty });
            // Update estimationLibrary
        ...
        const updateTemplate = await template.reload();
        return onSuccess(res, 200, "Successfully updated", updateTemplate);
      }
    } catch (error) {
  ...
  // re-calculate project data after adding estimation
    ...
    try {
      ...
      if (role !== "manager")
        return onError(
          res,
          401,
          "You are not allowed to approve or disapprove project"
        );
  ...
  static async addProjectActivity(req, res) {
    ...
    try {
      ...
      const estimation = await UserEstimations.findAll({
        where: {
          userEstimationCategoryId: defaultCategory.id,
          userId: managerId ? managerId : userId,
        },
        include: [
          {
  ...
  // remove activity Source project
    ...
    try {
      ...
      if (role === "manager" || role === "admin") {
        ...
        if (!estimation)
          return onError(
            res,
            400,
            "Estimation does not exist within this project"
          );
        return onSuccess(
          res,
          200,
      ...
      if (role === "owner") {
        const estimation = await Estimations.findOne({
          where: { templateId: req.params.projectId, id: req.params.id },
          include: [
            {
              model: EstimationCategory,
              as: "category",
        ...
        if (!estimation) return onError(res, 404, "Estimation does not exist");
        return onSuccess(
          res,
          200,
          "Estimation returned successfully",
  ...
  static async addEstimationToProject(req, res) {
    ...
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        if (userEstimation) {
          ...
          if (!estimationExistCategory) {
            ...
            categoryId = estimationExistCategory.id
          }

          for(let sub of subcategory){
            await UserEstimationSubcategory.create({
              name: sub.name,
              code: sub.name,
          ...
          for (let i = 0; i < equipments.length; i++) {
            ...
            const code = currency ? currency : "RWF";
            if (!userEquipment) {
              ...
              let newAmount;
              if (+library?.equipmentTotalAmount > 0) {
                newAmount = new CC({
                  from: code,
                  to: currencyCode,
                  amount: +library.equipmentTotalAmount,
                });
              }
              const newResponse = await newAmount?.convert();
              // create new UserEstimationLibary
              await UserEstimationLibrary.create(
                {
                  equipmentId: newEquipment.id,
                  equipmentPerformance: library?.equipmentPerformance
                    ? library?.equipmentPerformance
                    : 0,
                  equipmentTotalAmount: newResponse ? newResponse : 0,
                  estimationId: newEstimation.id,
                  userId: managerId ? managerId : userId,
                  projectId,
                },
          ...
          for (let i = 0; i < materials.length; i++) {
            ...
            if (!userMaterial) {
              ...
              let newAmount;
              if (+library?.materialTotalAmount > 0) {
                newAmount = new CC({
                  from: code,
                  to: currencyCode,
                  amount: +library.materialTotalAmount,
                });
              }
              const newResponse = await newAmount?.convert();
              // create new UserEstimationLibary
              await UserEstimationLibrary.create(
                {
                  materialId: newMaterial.id,
                  materialFactorQuantity: library?.materialFactorQuantity
                    ? library?.materialFactorQuantity
                    : 0,
                  materialTotalAmount: newResponse ? newResponse : 0,
                  estimationId: newEstimation.id,
                  userId: managerId ? managerId : userId,
                  projectId,
                },
          ...
          for (let i = 0; i < labours.length; i++) {
            ...
            if (!userLabour) {
              ...
              let newAmount;
              if (+library?.labourTotalAmount > 0) {
                newAmount = new CC({
                  from: code,
                  to: currencyCode,
                  amount: +library.labourTotalAmount,
                });
              }
              const newResponse = await newAmount?.convert();
              ...
              await UserEstimationLibrary.create(
                {
                  labourId: newLabour.id,
                  labourFactorQuantity: library?.labourFactorQuantity
                    ? library?.labourFactorQuantity
                    : 0,
                  labourTotalAmount: newResponse ? newResponse : 0,
                  estimationId: newEstimation.id,
                  userId: managerId ? managerId : userId,
                  projectId,
                },
          ...
          for (let i = 0; i < subContractors.length; i++) {
            ...
            if (!userSubContractor) {
              ...
              let newAmount;
              if (+library?.subContractorTotalAmount > 0) {
                newAmount = new CC({
                  from: code,
                  to: currencyCode,
                  amount: +library.subContractorTotalAmount,
                });
              }
              const newResponse = await newAmount?.convert();
              ...
              await await UserEstimationLibrary.create(
                {
                  subContractorId: newSubContractor.id,
                  subContractorTotalAmount: newResponse ? newResponse : 0,
                  estimationId: newEstimation.id,
                  userId: managerId ? managerId : userId,
                  projectId,
                },
          ...
          const addEstima = await UserEstimations.findOne({
            where: {id: newEstimation.id},
             include: [
            {
              model: UserEstimationCategory,
              as: "category",
          ...
          let wastageTotalConvert;
          let transportTotalConvert;
          let profitTotalConvert;
          let overHeadTotalConvert;
          let contigencyTotalConvert;
          let subtotalConvert;
          let estimationRateConvert;
          let estimationTotalAmountConvert;
          let equipmentCostPerWorkItemConvert;
          let equipmentRatePerUnitConvert;
          let materialCostPerWorkItemConvert;
          let materialRatePerUnitConvert;
          let indirectCostPerWorkItemConvert;
          let indirectRatePerUnitConvert;
          let labourCostperWorkItemConvert;
          let labourRatePerUnitConvert;
          let subcontractorCostPerWorkItemConvert;

          if (+wastageTotal > 0) {
            wastageTotalConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +wastageTotal,
            });
          }
          const wastageResponse = await wastageTotalConvert?.convert();

          if (+transportTotal > 0) {
            transportTotalConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +transportTotal,
            });
          }
          const transportResponse = await transportTotalConvert?.convert();

          if (+profitTotal > 0) {
            profitTotalConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +profitTotal,
            });
          }
          const profitResponse = await profitTotalConvert?.convert();

          if (+overHeadTotal > 0) {
            overHeadTotalConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +overHeadTotal,
            });
          }
          const overheadResponse = await overHeadTotalConvert?.convert();

          if (+contigencyTotal > 0) {
            contigencyTotalConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +contigencyTotal,
            });
          }
          const contigencyResponse = await contigencyTotalConvert?.convert();

          if (+subtotal > 0) {
            subtotalConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +subtotal,
            });
          }
          const subtotalResponse = await subtotalConvert?.convert();

          if (+estimationRate > 0) {
            estimationRateConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +estimationRate,
            });
          }
          const estimationRateResponse = await estimationRateConvert?.convert();

          if (+estimationTotalAmount > 0) {
            estimationTotalAmountConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +estimationTotalAmount,
            });
          }
          const estimationTotalAmountResponse =
            await estimationTotalAmountConvert?.convert();

          if (+equipmentCostPerWorkItem > 0) {
            equipmentCostPerWorkItemConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +equipmentCostPerWorkItem,
            });
          }
          const equipmentCostPerWorkItemConvertResponse =
            await equipmentCostPerWorkItemConvert?.convert();

          if (+equipmentRatePerUnit > 0) {
            equipmentRatePerUnitConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +equipmentRatePerUnit,
            });
          }
          const equipmentRatePerUnitConvertResponse =
            await equipmentRatePerUnitConvert?.convert();

          // material
          if (+materialCostPerWorkItem > 0) {
            materialCostPerWorkItemConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +materialCostPerWorkItem,
            });
          }
          const materialCostPerWorkItemConvertResponse =
            await materialCostPerWorkItemConvert?.convert();

          if (+materialRatePerUnit > 0) {
            materialRatePerUnitConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +materialRatePerUnit,
            });
          }
          const materialRatePerUnitConvertResponse =
            await materialRatePerUnitConvert?.convert();

          // indirect cost item
          if (+indirectCostPerWorkItem > 0) {
            indirectCostPerWorkItemConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +indirectCostPerWorkItem,
            });
          }
          const indirectCostPerWorkItemConvertResponse =
            await indirectCostPerWorkItemConvert?.convert();

          if (+indirectRatePerUnit > 0) {
            indirectRatePerUnitConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +indirectRatePerUnit,
            });
          }
          const indirectRatePerUnitConvertResponse =
            await indirectRatePerUnitConvert?.convert();

          // labour
          if (+labourCostperWorkItem > 0) {
            labourCostperWorkItemConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +labourCostperWorkItem,
            });
          }
          const labourCostperWorkItemConvertResponse =
            await labourCostperWorkItemConvert?.convert();

          if (+labourRatePerUnit > 0) {
            labourRatePerUnitConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +labourRatePerUnit,
            });
          }
          const labourRatePerUnitConvertResponse =
            await labourRatePerUnitConvert?.convert();

          // subcontractor

          if (+subcontractorCostPerWorkItem > 0) {
            subcontractorCostPerWorkItemConvert = new CC({
              from: "RWF",
              to: currencyCode,
              amount: +subcontractorCostPerWorkItem,
            });
          }
          const subcontractorCostPerWorkItemConvertResponse =
            await subcontractorCostPerWorkItemConvert?.convert();
          ...
          const newEstimation = await UserEstimations.create(
            {
              ...
              estimationQuantity,
              wastagePercentage,
              wastageTotal: wastageResponse,
              transportPercentage,
              transportTotal: transportResponse,
              profitPercentage,
              profitTotal: profitResponse,
              overHeadPercentage,
              overHeadTotal: overheadResponse,
              contigencyPercentage,
              contigencyTotal: contigencyResponse,
              subtotal: subtotalResponse,
              estimationRate: estimationRateResponse,
              estimationTotalAmount: estimationTotalAmountResponse,
              userEstimationCategoryId: categoryId,
              equipmentCostPerWorkItem: equipmentCostPerWorkItemConvertResponse,
              equipmentRatePerUnit: equipmentRatePerUnitConvertResponse,
              materialCostPerWorkItem: materialCostPerWorkItemConvertResponse,
              materialRatePerUnit: materialRatePerUnitConvertResponse,
              indirectCostPerWorkItem: indirectCostPerWorkItemConvertResponse,
              indirectRatePerUnit: indirectRatePerUnitConvertResponse,
              labourCostperWorkItem: labourCostperWorkItemConvertResponse,
              labourRatePerUnit: labourRatePerUnitConvertResponse,
              subcontractorCostPerWorkItem:
                subcontractorCostPerWorkItemConvertResponse,
              projectId,
              userId: managerId ? managerId : userId,
            },
            { transaction: t }
      ...
      if (role === "owner") {
        ...
        const defaultEstimation = await Estimations.findOne({
          where: { name: estimationName, templateId: null},
          include: [
            {
              model: EstimationCategory,
              as: "category",
        ...
        if (estimationExists) {
          return onError(res, 409, "Estimation already added");
        }
        ...
        // estimation category exist
        const subcategory = category?.subcategory;
        const estimationExistCategory = await EstimationCategory.findOne({
          where: {
            name: category?.name,
        ...
        let categoryId;
        if (!estimationExistCategory) {
          const created = await EstimationCategory.create({
            name: category?.name,
            templateId: projectId,
            code: category?.name,
          });
        ...
        for(let sub of subcategory){
            await EstimationSubcategory.create({
              name: sub.name,
              code: sub.name,
              mainCategoryId: categoryId
            })
        ...
        for (let i = 0; i < equipments.length; i++) {
          ...
          const userEquipment = await Equipments.findOne({
            where: { name, templateId: projectId },
          });
          if (!userEquipment) {
            // create new equipment
            ...
            await EstimationLibrary.create(
              {
                ...
                equipmentPerformance: library?.equipmentPerformance
                  ? library?.equipmentPerformance
                  : 0,
                equipmentTotalAmount: library?.equipmentTotalAmount
                  ? library?.equipmentTotalAmount
                  : 0,
                estimationId: newEstimation.id,
                templateId: projectId,
              },
        ...
        for (let i = 0; i < materials.length; i++) {
          ...
          const userMaterial = await Materials.findOne({
            where: { name, templateId: projectId },
          });
            ...
            await EstimationLibrary.create(
              {
                ...
                materialFactorQuantity: library?.materialFactorQuantity
                  ? library?.materialFactorQuantity
                  : 0,
                materialTotalAmount: library?.materialTotalAmount
        ...
        for (let i = 0; i < labours.length; i++) {
          ...
          const userLabour = await Labours.findOne({
            where: { name, templateId: projectId },
          });
            ...
            await EstimationLibrary.create(
              {
                ...
                labourFactorQuantity: library?.labourFactorQuantity
                  ? library?.labourFactorQuantity
                  : 0,
                labourTotalAmount: library?.labourTotalAmount
                  ? library?.labourTotalAmount
                  : 0,
                estimationId: newEstimation.id,
                templateId: projectId,
              },
        ...
        for (let i = 0; i < subContractors.length; i++) {
          ...
          const userSubContractor = await SubContractors.findOne({
            where: { name, templateId: projectId },
          });
          ...
          if (!userSubContractor) {
            ...
            await await EstimationLibrary.create(
              {
                subContractorId: newSubContractor.id,
                subContractorTotalAmount: library?.subContractorTotalAmount
                  ? library?.subContractorTotalAmount
                  : 0,
                estimationId: newEstimation.id,
                templateId: projectId,
              },
        ...
        const addEstima = await Estimations.findOne({
            where: {id: newEstimation.id},
             include: [
            {
              model: EstimationCategory,
  ...
  // remove estimation Source project
  ...
  static async removeEstimationFromProject(req, res) {
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        const updated = await project.reload();
        return onSuccess(res, 200, "Estimation Removed Successfully", updated);
      }

      if (role === "owner") {
        const project = await Templates.findOne({
          where: { id: projectId },
        });
        ...
        if (!estimationExists) {
          return onError(res, 409, "Estimation Doesn't Exist");
        }

        // remove estimation Source project

        await Estimations.destroy({
          where: { id: estimationExists.id },
        });
  ...
  static async copyProjectData(req, res) {
    ...
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        for (let estimation of estimations) {
          const {
            ...
            labours,
            subContractors,
            category: { name: categoryName, code },
          } = estimation;
          const created = await UserEstimationCategory.create({
            name: categoryName,
      ...
      if (role === "owner") {
        const project = await Templates.findOne({ where: { id } });
        ...
        let projectName = project.name;
        const projectExists = await Templates.findOne({
          where: { name: project.name },
        });
        ...
        const newProject = await Templates.create(
          {
            name: projectName,
            description: project?.description,
            status: project?.status,
            ...
            currencyValue: project?.currencyValue,
          },
          { transaction: t }
        );
        ...
        for (let estimation of estimations) {
          ...
          for (let i = 0; i < equipments.length; i++) {
            const {
              ...
              number,
              supplierId,
              EstimationLibrary: { equipmentPerformance, equipmentTotalAmount },
            } = equipments[i];
          ...
          for (let i = 0; i < labours.length; i++) {
            const {
              ...
              wages,
              caveragePerUnit,
              EstimationLibrary: { labourFactorQuantity, labourTotalAmount },
            } = labours[i];
          ...
          for (let i = 0; i < subContractors.length; i++) {
            const {
              ...
              quantity,
              price,
              EstimationLibrary: { subContractorTotalAmount },
            } = subContractors[i];
        ...
        return onSuccess(res, 200, "Copied successfully", newProject);
      }
    } catch (error) {
  ...
  // copy estimation from one project to another;

  static async copyEstimationFromProjectToAnother(req, res){
    const t = await sequelize.transaction();
    try {
      const { id: userId, managerId, role } = req.user;
      const {fromProjectId, toProjectId, destinationCategoryId} = req.body;
      const { id } = req.params

      if(role === 'manager' || role === 'admin'){
        const fromProject = await Projects.findOne({ where: { id: fromProjectId  } });
        const toProject = await Projects.findOne({ where: { id: toProjectId  } });
        if (!fromProject) {
          return onError(res, 404, "Source project not found");
        }

        if (!toProject) {
          return onError(res, 404, "Destination project not found");
        }
        ...
        if(!category) {
          return onError(res, 404, 'destination category not found')
        }

        const estimation = await UserEstimations.findOne({
          where: { userId: managerId ? managerId : userId, id, projectId: fromProjectId },
          order: [["id", "ASC"]],
          include: [
            {
              model: UserEstimationCategory,
        ...
        return onSuccess(
          res,
          200,
          "Estimation copied successfully",
          newEstimation
        );

      }
      if(role === 'owner'){
        const fromProject = await Templates.findOne({ where: { id: fromProjectId  } });
        const toProject = await Templates.findOne({ where: { id: toProjectId  } });
        if (!fromProject) {
          return onError(res, 404, "Source template not found");
        }

        if (!toProject) {
          return onError(res, 404, "Destination template not found");
        }

        const category = await EstimationCategory.findOne({
          where: {
            id: destinationCategoryId,
            templateId: toProjectId,
          },
        })
        ...
        const estimation = await Estimations.findOne({
          where: { id, templateId: fromProjectId },
          order: [["id", "ASC"]],
          include: [
            {
              model: EstimationCategory,
        ...
        const newEstimation = await Estimations.create(
          {
            ...
            estimationRate,
            estimationTotalAmount,
            estimationCategoryId: category?.id,
            equipmentCostPerWorkItem,
            equipmentRatePerUnit,
            materialCostPerWorkItem,
            materialRatePerUnit,
  ...
  static async createProjectFromTemplate(req, res) {
    const t = await sequelize.transaction();
    try {
      const { id: userId, managerId, role } = req.user;
      const { id } = req.params;
      ...
      if (role === "admin") {
        ...
        return onSuccess(res, 200, "Project created successfully", newProject);
      }
      if (role === "manager") {
        const newProject = await Projects.create(
          {
  ...
  static async updateEstimation(req, res) {
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        const editedRate = editedEstimationRate
          ? editedEstimationRate
          : +estimation.estimationRate;


        // edited rate
        if (
          editedEstimationRate ||
          (editedEstimationRate && estimationQuantity)
        ) {
          for (let i = 0; i < equipments.length; i++) {
            const equipment = equipments[i];
            // delete calculation
          ...
          const totalAmount = editedEstimationRate * estimationQty;
          // update estimation
          await estimation.update({
            name: editedName,
            estimationRate: editedEstimationRate,
            estimationQuantity: estimationQty,
            ...
            contigencyTotal: 0,
            estimationTotalAmount: totalAmount.toFixed(2),
            materialCostPerWorkItem: 0,
            materialRatePerUnit: 0,
            equipmentCostPerWorkItem: 0,
            equipmentRatePerUnit: 0,
            indirectCostPerWorkItem: 0,
            indirectRatePerUnit: 0,
            labourCostperWorkItem: 0,
            labourRatePerUnit: 0,
            subcontractorCostPerWorkItem: 0,
          });
        ...
        for (let i = 0; i < labours.length; i++) {
          ...
          const calculatedWages =
            +labor.number * +labor.wages * labourFactorQuantity;
          const labourTotalAmount = calculatedWages;
        ...
        const calculatedOverHead =
          +estimation?.estimationRate * estimationQty * overHead;
        const tempProfit =
          +estimation?.estimationRate * estimationQty + calculatedOverHead;
        const calculatedProfit = tempProfit * profit;
        const tempContigency =
          +estimation?.estimationRate * estimationQty + calculatedProfit;
        const calculatedContigency = tempContigency * contigency;
  ...
  static async deleteProject(req, res) {
    try {
      ...
      if (role === "owner") {
        ...
        const projectData = { ...template.dataValues, activities: estimations };
        const { activities } = projectData;
        ...
        for (let j = 0; j < activities.length; j++) {
          ...
          await EstimationCategory.destroy({
            where: { id: userEstimationCategoryId },
          });
        }
        ...
        await Labours.destroy({
          where: { templateId: id },
        });
        await SubContractors.destroy({
          where: { templateId: id },
        });

        await template.destroy();

        return onSuccess(res, 200, "Deleted Successfully", template);
      }
    } catch (error) {
      return onError(
        res,
  ...
  static async addEquipmentToProjectEstimation(req, res) {
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        if (!project) return onError(res, 404, "Project Not Found");
        const currencyCode = project?.currencyCode;
        const estimation = await UserEstimations.findOne({
          where: { id, userId: managerId ? managerId : userId, projectId },
          include: [
        ...
        if (!equipment) {
          return onError(
            res,
            404,
            "Resource not found, add this resource to this project"
          );
        }
        ...
        const equipExt = await UserEquipments.findOne({
          where: {
            name: equipment.name,
            projectId,
            isResource: false,
        ...
        if (equipExt) {
          const appliedPrice = +equipExt.hireRatePrice;
          const currency = equipExt?.currency ? equipExt?.currency : "RWF";
          ...
          const createdEquipment = await UserEquipments.create({
            name: equipExt.name,
            unit: equipExt.unit,
            caveragePerUnit: equipExt.caveragePerUnit,
            hireRatePrice: finalPrice,
            number: equipExt.number,
            supplierId: equipExt?.supplierId,
            userId: managerId ? managerId : userId,
            projectId,
            currency,
            isResource: false,
            inputQuantity: equipExt?.inputQuantity,
            outputQuantity: equipExt?.outputQuantity,
            inputUnit: equipExt?.inputUnit,
            outputUnit: equipExt?.outputUnit,
            adjustFactor: equipExt?.adjustFactor,
          });

          const inputQuantity = +returnNumber(createdEquipment?.inputQuantity);
          const outputQuantity = +returnNumber(createdEquipment?.outputQuantity);
          const adjustFactor = +returnNumber(createdEquipment?.adjustFactor);
          ...
          const calculatedPrice =
            ...
            +createdEquipment.hireRatePrice *
            equipmentPerformance;
          const equipmentTotalAmount = calculatedPrice;
          ...
          const calculatedOverHead =
            ...
            +estimation?.overHeadPercentage;
          const tempProfit =
            +estimation?.estimationRate * +estimation.estimationQuantity +
            calculatedOverHead;
          ...
          const calculatedContigency =
            tempContigency * +estimation?.contigencyPercentage;
        ...
        const appliedPrice = +equipment.hireRatePrice;
        const currency = equipment?.currency ? equipment?.currency : "RWF";
        ...
        const createdEquipment = await UserEquipments.create({
          ...
          currency,
          isResource: false,
          inputQuantity: equipment?.inputQuantity,
          outputQuantity: equipment?.outputQuantity,
          inputUnit: equipment?.inputUnit,
          outputUnit: equipment?.outputUnit,
          adjustFactor: equipment?.adjustFactor,
        });
      ...
      if (role === "owner") {
        ...
        if (!project) return onError(res, 404, "Templates Not Found");
        const currencyCode = project?.currencyCode;
        const estimation = await Estimations.findOne({
          where: { id, templateId: projectId },
          include: [
        ...
        if (defaultEquipment) {
          ...
          const existingEqp = await Equipments.findOne({
            where: {
          ...
          if (existingEqp) {
            const appliedPrice = +existingEqp.hireRatePrice;
            const currency = existingEqp?.currency
              ? existingEqp?.currency
              : "RWF";
            let currencyConverter;
            if (appliedPrice > 0) {
              currencyConverter = new CC({
            ...
            const createdEquipment = await Equipments.create({
              name: existingEqp.name,
              unit: existingEqp.unit,
              caveragePerUnit: existingEqp.caveragePerUnit,
              hireRatePrice: finalPrice,
              number: existingEqp.number,
              supplierId: existingEqp?.supplierId,
              templateId: projectId,
              currency,
              inputQuantity: existingEqp?.inputQuantity,
              outputQuantity: existingEqp?.outputQuantity,
              inputUnit: existingEqp?.inputUnit,
              outputUnit: existingEqp?.outputUnit,
              adjustFactor: existingEqp?.adjustFactor,
            });
            ...
            const subcontractorTotal = subContractors.reduce((acc, curr) => {
              return (
                acc + +curr.UserEstimationLibrary?.subContractorTotalAmount
              );
            }, 0);
            ...
            const appliedPrice = +defaultEquipment.hireRatePrice;
            const currency = defaultEquipment?.currency
              ? defaultEquipment?.currency
              : "RWF";
            ...
            const createdEquipment = await Equipments.create({
              ...
              adjustFactor: defaultEquipment?.adjustFactor,
            });
  ...
  static async editProjectEstimationEquipment(req, res) {
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        await calculateSimilarProjectEquipment(projectId);

        const updateed = await estimation.reload();
        return onSuccess(res, 200, "Equipment Updated successfully", updateed);
      }
      ...
      if (role === "owner") {
        ...
        const upd = await Equipments.update(
          {
            name: editEquipmentName,
            unit: editEquipmentUnit,
            hireRatePrice: equipPrice,
       ...
       await equipment.update({
        ...
        await calculateSimilarTemplateEquipment(projectId);
        const updateed = await estimation.reload();
        return onSuccess(res, 200, "Equipment Updated successfully", updateed);
      }
    } catch (error) {
    ...
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        if (defaultmaterial) {
          ...
          const matExt = await UserMaterials.findOne({
            where: {
          ...
          if (matExt) {
            const appliedPrice = +matExt.price;
            const currency = matExt?.currency ? matExt?.currency : "RWF";
            let currencyConverter;
            if (appliedPrice > 0) {
              currencyConverter = new CC({
                from: currency,
            ...
            const createdMaterial = await UserMaterials.create({
              name: matExt.name,
              unit: matExt.unit,
              caveragePerUnit: matExt.caveragePerUnit,
              quantity: matExt.quantity,
              price: finalPrice,
              supplierId: matExt?.supplierId,
              userId: managerId ? managerId : userId,
              projectId,
              currency,
              isResource: false,
          ...
          const appliedPrice = +defaultmaterial.price;
          const currency = defaultmaterial?.currency
            ? defaultmaterial?.currency
            : "RWF";
          let currencyConverter;
          if (appliedPrice > 0) {
            currencyConverter = new CC({
        ...
        // formaulas material
        const mtexist = await UserMaterials.findOne({
          where: {
            name: material.name,
            projectId,
          },
        ...
        if (mtexist) {
          const appliedPrice = +mtexist.price;
          const currency = mtexist?.currency ? mtexist?.currency : "RWF";
          let currencyConverter;
          if (appliedPrice > 0) {
            currencyConverter = new CC({
              from: currency,
          ...
          const createdMaterial = await UserMaterials.create({
            name: mtexist.name,
            unit: mtexist.unit,
            caveragePerUnit: mtexist.caveragePerUnit,
            quantity: mtexist.quantity,
            price: finalPrice,
            supplierId: mtexist?.supplierId,
            userId: managerId ? managerId : userId,
            projectId,
            currency,
            isResource: false,
        ...
        const appliedPrice = +material.price;
        const currency = material?.currency ? material?.currency : "RWF";
        let currencyConverter;
        if (appliedPrice > 0) {
          currencyConverter = new CC({
            from: currency,
        ...
        const createdMaterial = await UserMaterials.create({
          name: material.name,
          unit: material.unit,
          caveragePerUnit: material.caveragePerUnit,
      ...
      if (role === "owner") {
        ...
        if (!project) return onError(res, 404, "template not found");
        const currencyCode = project?.currencyCode;
        ...
        if (defaultmaterial) {
          // create UserMaterials from defaultMaterial
          const matExt = await Materials.findOne({
            where: {
              name: defaultmaterial.name,
              templateId: projectId,
            },
          ...
          if (matExt) {
            ...
            return onSuccess(res, 200, "Material added successfully", updates);
          }
  ...
  static async removeMaterialFromProjectEstimation(req, res) {
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        const updates = await updateEstimation.reload();
        return onSuccess(res, 200, "Material Removed successfully", updates);
      }
    ...
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        await material.update({
          ...
          outputUnit,
          outputQuantity,
          adjustFactor: caverage,
        });

        // make new calculation;

        await calculateSimilarProjectMaterial(projectId);

        const updated = await estimation.reload();

        return onSuccess(res, 200, "Material Updated successfully", updated);
      }
      ...
      if (role === "owner") {
        ...
        // make new calculation;
        await calculateSimilarTemplateMaterial(projectId);
        const updated = await estimation.reload();
    ...
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        if (defaultLabour) {
          ...
          const labExt = await UserLabours.findOne({
            where: {
          ...
          if (labExt) {
            const appliedPrice = +labExt.wages;
            const currency = labExt?.currency ? labExt?.currency : "RWF";
            let currencyConverter;
            if (appliedPrice > 0) {
              currencyConverter = new CC({
                from: currency,
            ...
            const createdLabour = await UserLabours.create({
              name: labExt.name,
              unit: labExt.unit,
              caveragePerUnit: labExt?.caveragePerUnit,
              number: labExt?.number,
              wages: finalPrice,
              userId: managerId ? managerId : userId,
              projectId,
              currency,
            ...
            // calculate Overhead, Contigency and Profit
            const calculatedOverHead =
              +updateEstimation?.estimationRate *
            ...
            const tempContigency =
              ...
              calculatedProfit;
            const calculatedContigency =
              tempContigency * +updateEstimation?.contigencyPercentage;
          ...
          const appliedPrice = +defaultLabour.wages;
          const currency = defaultLabour?.currency
          ...
          const labourFactorQuantity =
            +createdLabour?.caveragePerUnit * +estimation.estimationQuantity;
          const calculatedWages =
            +createdLabour.number * +createdLabour.wages * labourFactorQuantity;
          const labourTotalAmount = calculatedWages;
        ...
        const appliedPrice = +labour.wages;
        const currency = labour?.currency ? labour?.currency : "RWF";
        let currencyConverter;
        if (appliedPrice > 0) {
          currencyConverter = new CC({
            from: currency,
      ...
      if (role === "owner") {
        ...
        if (defaultLabour) {
          ...
          const labExt = await Labours.findOne({
            where: {
              name: defaultLabour.name,
              templateId: projectId,
            },
          ...
          if (labExt) {
            ...
            const createdLabour = await Labours.create({
              name: labExt.name,
              unit: labExt.unit,
              caveragePerUnit: labExt?.caveragePerUnit,
              number: labExt?.number,
            ...
            const updates = await updateEstimation.reload();
            return onSuccess(res, 200, "Labour added successfully", updates);
          }
  ...
  static async removeLabourFromProjectEstimation(req, res) {
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        if (!labourExist)
          return onError(
            res,
            404,
            "labour doesn't exist within this estimation"
          );
      ...
      if (role === "owner") {
        ...
        return onSuccess(res, 200, "Labour Removed successfully", updates);
      }
    } catch (error) {
  ...
  static async editLabourProjectEstimation(req, res) {
    try {
      const { labourName, wages, number, editName, editUnit, outputUnit,
        outputQuantity, adjustFactor } =
        req.body;
      ...
      if (role === "admin" || role === "manager") {
        ...
        // caverage per unit
        const caverage = adjustFactor
          ? adjustFactor
          : +labourExist.adjustFactor;
        ...
        // make new calculation;
        await calculateSimilarProjectLabour(projectId);

        const updated = await estimation.reload();
        return onSuccess(res, 200, "Labour Updated successfully", updated);
      }
      ...
      if (role === "owner") {
        ...
        // make new calculation;
        await calculateSimilarTemplateLabour(projectId);
    ...
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        if (default_subcontractor) {
          ...
          const subcoExt = await UserSubContractors.findOne({
            where: {
          ...
          if (subcoExt) {
            const appliedPrice = +subcoExt.price;
            const currency = subcoExt?.currency ? subcoExt?.currency : "RWF";
            let currencyConverter;
            if (appliedPrice > 0) {
              currencyConverter = new CC({
                from: currency,
          ...
          const appliedPrice = +default_subcontractor.price;
          const currency = default_subcontractor?.currency
            ? default_subcontractor?.currency
            : "RWF";
          let currencyConverter;
          if (appliedPrice > 0) {
            currencyConverter = new CC({
        ...
        const subcExt = await UserSubContractors.findOne({
          where: {
        ...
        if (subcExt) {
          const appliedPrice = +subcExt.price;
          const currency = subcExt?.currency ? subcExt?.currency : "RWF";
          let currencyConverter;
          if (appliedPrice > 0) {
            currencyConverter = new CC({
              from: currency,
        ...
        const appliedPrice = +subcontractor.price;
        const currency = subcontractor?.currency
          ? subcontractor?.currency
          : "RWF";
        let currencyConverter;
        if (appliedPrice > 0) {
          currencyConverter = new CC({
      ...
      if (role === "owner") {
        ...
        if (default_subcontractor) {
          // create UserLabours from default_subcontractor
          const subcoExt = await SubContractors.findOne({
            where: {
              name: default_subcontractor.name,
              templateId: projectId,
            },
          ...
          if (subcoExt) {
            ...
            const created_subcontractor = await SubContractors.create({
              name: subcoExt.name,
              unit: subcoExt.unit,
              quantity: +estimation.estimationQuantity,
              price: finalPrice,
  ...
  static async editSubcontractorProjectEstimation(req, res) {
    try {
      ...
      if (role === "admin" || role === "manager") {
        ...
        await UserSubContractors.update(
          {
            price: subc_price,
        ...
        // make new calculation;
        await calculateSimilarProjectSubcontractor(projectId);
      ...
      if (role === "owner") {
        ...
        // update equipment
        await calculateSimilarTemplateSubcontractor(projectId);
  ...
  // upload boq csv
  static async importBOQViaCSV(req, res) {
     const t = await sequelize.transaction();
    try {
      const { id: userId, role, managerId } = req.user;
      const { projectId } = req.params;
      if (!req.files) return onError(res, 400, "csv file is required");

      const { csvFile } = req.files;
      // TEMPORARY PATH
      const csvTempPath = csvFile.tempFilePath;

      // check if file is a csv
      const csvFileExtension = csvFile.name.split(".")[1];
      if (csvFileExtension !== "csv") {
        return onError(res, 400, "Please enter a csv file");
      }

      const jsonArray = await csvtojson().fromFile(csvTempPath);
      ...
      for (let i = 0; i < jsonArray.length; i++) {
        ...
        if (!data.description) {
          ...
          return onError(res, 400, "description is required and header must be written as 'description'");
        }
        
        if (!data.unit) {
          return onError(res, 400,"unit is required and header must be written as 'Unit'"
          );
        }
        if (!data.category) {
          return onError(res, 400, "main category name is required and header must be written as 'mategory'");
        }
      }
      ...
      if (role === "manager" || role === "admin") {
        ...
        // import boq
        for (let i = 0; i < jsonArray.length; i++) {
          const boq = jsonArray[i];
          const description = boq?.description?.toLowerCase();
          const unit = boq?.unit?.toLowerCase();
          const quantity = boq?.quantity ? +boq?.quantity : 0;
          const category = boq?.category?.toLowerCase();
          const price = boq?.price ? +boq.price : 0;
          const code = boq?.code?.toLowerCase();

          const calculatedAmount = quantity * price
  
          const categoryCreated = await UserEstimationCategory.create({
              name: category,
              code,
              userId: managerId ? managerId : userId,
              projectId,
          ...
          // activtyCreated
          const createdEstimation = await UserEstimations.create({
            name: description,
            estimationUnit: unit,
            estimationQuantity: quantity,
            userEstimationCategoryId: categoryCreated.id,
            projectId,
            userId: managerId ? managerId : userId,
            estimationRate: price,
            estimationTotalAmount:calculatedAmount.toFixed(2),
          }, { transaction: t })
  
        }
  
         await t.commit();
        return onSuccess(res, 200, "BOQ have been successfully added, select project again");
      ...
      if(role === 'owner'){
        ...
        for (let i = 0; i < jsonArray.length; i++) {
          ...
          const categoryCreated = await EstimationCategory.create({
              name: category,
              code,
              templateId: projectId
          }, { transaction: t });
          ...
          const createdEstimation = await Estimations.create({
            ...
            userId: managerId ? managerId : userId,
            estimationRate: price,
            estimationTotalAmount:calculatedAmount,
          }, { transaction: t })
  ...
  // map estimation to another project
  static async mapEstimationToAnotherEstimation(req, res){
    try{
      const { id: userId, managerId, role } = req.user;
      const {projectId: destinationProjectId} = req.params
      const {sourceProjectId, sourceEstimationId, destinationEstimationId} = req.body;

      if(role === 'admin' || role === 'manager'){
        const sourceproject = await Projects.findOne({
          where: { id: sourceProjectId}
      })

      const destinationProject = await Projects.findOne({
      ...
      const sourceest1 = await UserEstimations.findOne({
          where: { id: sourceEstimationId, projectId: sourceproject.id},
          include: [
            {
              model: UserMaterials,
              as: "materials",
      ...
      const destiest2 = await UserEstimations.findOne({
          where: { id: destinationEstimationId, projectId: destinationProjectId},
          include: [
            {
              model: UserMaterials,
              as: "materials",
      ...
      if(!sourceest1) {
        return onError(res, 404, 'source activity not found')
      }
      if(!destiest2) {
        return onError(res, 404, 'destination activity not found')
      }
      const { equipments,materials,labours, subContractors } = sourceest1;
      const { equipments:desEquipment,materials: dseMaterial ,labours: dseLabours, subContractors: dseSub } = destiest2;

      // delete existing resources

      // delete equipments
      for(let i = 0; i<desEquipment.length; i++){
        const id = desEquipment[i].id;
        ...
        await UserEstimationLibrary.destroy({
          where: { equipmentId: id, estimationId: destinationEstimationId,  }
        })
      }

      // delete materials
      for(let i = 0; i<dseMaterial.length; i++){
        const id = dseMaterial[i].id;
        ...
        await UserEstimationLibrary.destroy({
          where: { materialId: id, estimationId: destinationEstimationId,  }
        })
      }

      // delete labours
      for(let i = 0; i<dseLabours.length; i++){
        const id = dseLabours[i].id;
        ...
        await UserEstimationLibrary.destroy({
          where: { labourId: id, estimationId: destinationEstimationId,  }
        })
      }

      // delete subcontractor
      for(let i = 0; i<dseSub.length; i++){
        const id = dseSub[i].id;
        ...
        await UserEstimationLibrary.destroy({
          where: { subContractorId: id, estimationId: destinationEstimationId,  }
        })
      }
      ...
      // loop through equipment
          ...
          await UserEstimationLibrary.create(
            {
              ...
              equipmentTotalAmount: library?.equipmentTotalAmount
                ? library?.equipmentTotalAmount
                : 0,
              estimationId: destinationEstimationId,
              userId: managerId ? managerId : userId,
            },
          );
        }
        ...
        await calculateSimilarProjectEquipment(destinationProjectId),
        await calculateSimilarProjectMaterial(destinationProjectId)
        await calculateSimilarProjectLabour(destinationProjectId)
        await calculateSimilarProjectSubcontractor(destinationProjectId)

        const updates = await destiest2.reload();

        return onSuccess(
          res,
          200,
          "resources mapped successfully",
          updates
        );
      ...
      if(role === 'owner'){
        const sourceproject = await Templates.findOne({
          where: { id: sourceProjectId}
      })

      const destinationProject = await Templates.findOne({
          where: { id: destinationProjectId}
      })
      ...
      const sourceest1 = await Estimations.findOne({
          where: { id: sourceEstimationId, templateId: sourceproject.id},
          include: [
            {
              model: Materials,
              as: "materials",
      ...
      const destiest2 = await Estimations.findOne({
          where: { id: destinationEstimationId, templateId: destinationProjectId},
          include: [
            {
              model: Materials,
              as: "materials",
          ...
          const newEquipment = await Equipments.create(
            {
              ...
              number,
              supplierId,
              templateId: destinationProjectId,
              currency,
            },
          );
          // create new UserEstimationLibary
