...
class UserEstimationController {
  ...
  static async createEstimation(req, res) {
    try {
      ...
      if (role === "manager" || role === "admin") {
        ...
        if (defaultCategory) {
          ...
          if (userCategory) {
            ...
            const ourEstimation = await Estimations.findOne({
              where: { name: estimation.name },
            });
            if (!ourEstimation) {
              const categoryExt = await EstimationCategory.findOne({
                where: { name: userCategory.name },
              });
          ...
          const createdCategory = await UserEstimationCategory.create({
            name: defaultCategory.name,
            userId: managerId ? managerId : userId,
          });
          const defaultCaty = await EstimationCategory.findOne({
            where: { name: createdCategory.name },
          });
          if (!defaultCaty) {
            const makeCategoryExst = await EstimationCategory.create({
              name: category.name,
              userId: managerId ? managerId : userId,
              isApproved: false,
            });
      ...
      const estimationExist = await Estimations.findOne({
        where: { name: lowerCaseName, templateId: null },
      });
      if (estimationExist) {
        return onError(res, 409, "Estimation name already exist");
      }
  ...
  static async updateEstimation(req, res) {
    try {
      ...
      if (role === "owner") {
        ...
        return onSuccess(res, 200, "Estimation updated successfully", updates);
      }
    } catch (error) {
      return onError(
        res,
  ...
  static async addEquipmentToEstimation(req, res) {
    try {
        ...
        if (defaultEquipment) {
          ...
          if (existingEquipment) {
            ...
            const calculatedPriceAmount =
              ...
              +existingEquipment.hireRatePrice *
              equipmentPerformance;
            const equipmentTotalAmount = calculatedPriceAmount;
      ...
      const equipmentExist = estimation.equipments.find(
        (equip) => equip.name === equipmentName
      );
  ...
  static async removeEquipmentFromEstimation(req, res) {
    try {
      ...
      if (role === "manager" || role === "admin") {
        ...
        return onSuccess(res, 200, "Equipment Removed successfully", updates);
      }
      const estimation = await Estimations.findOne({
        where: { id },
        include: [
  ...
  static async editEstimationEquipment(req, res) {
    try {
      ...
      if (role === "manager" || role === "admin") {
        ...
        return onSuccess(res, 200, "Equipment Updated successfully", updates);
      }
      const estimation = await Estimations.findOne({
        where: { id: req.params.id },
        include: [
      ...
      if (role !== "owner") {
        ...
        if (defaultmaterial) {
          ...
          if (existingMaterial) {
            ...
            const materialFactorQuantity =
              +existingMaterial?.caveragePerUnit *
              +estimation.estimationQuantity;
            ...
            const materialFactorQuantity =
              +createdMaterial?.caveragePerUnit *
              +estimation.estimationQuantity;
  ...
  static async editEstimationMaterials(req, res) {
    try {
      ...
      if (role !== "owner") {
        ...
        const updates = await updateEstimation.reload();
        return onSuccess(res, 200, "Material Updated successfully", updates);
      }
      const estimation = await Estimations.findOne({
        where: { id: req.params.id },
        include: [
  ...
  static async addLabourToEstimation(req, res) {
    try {
        ...
        if (defaultLabour) {
          ...
          if (existingLabour) {
            ...
            const labourFactorQuantity =
              +existingLabour?.caveragePerUnit * +estimation.estimationQuantity;
            const calculatedWages =
              +existingLabour?.number *
              existingLabour?.wages *
              labourFactorQuantity;
      ...
      return onSuccess(res, 200, "Labour added successfully", updateEstimation);
    } catch (error) {
      return onError(
        res,
        500,
  ...
  static async editLabourEstimation(req, res) {
    try {
      const { caveragePerUnit, labourName, editName, editUnit, wages, number } =
        req.body;
      ...
      if (role !== "owner") {
        ...
        for (let i = 0; i < labours.length; i++) {
          ...
          const calculatedWage =
            +labour.wages * labourNumber * labourFactorQuantity;
          const labourTotalAmount = calculatedWage;
        ...
        const updates = await updateEstimation.reload();
        return onSuccess(res, 200, "Labour Updated successfully", updates);
      }
      const estimation = await Estimations.findOne({
        where: { id: req.params.id },
        include: [
  ...
  static async removeSubcontractorFromEstimation(req, res) {
    try {
      ...
      const updates = await updateEstimation.reload();
      return onSuccess(res, 200, "Subcontractor Removed successfully", updates);
    } catch (error) {
      console.log("error: ", error);
      return onError(
        res,
    ...
    try {
      ...
      if (role !== "owner") {
        ...
        for (let i = 0; i < subContractors.length; i++) {
          ...
          const subContractorTotalAmount =
            +updateEstimation.estimationQuantity * +subco.price;
      ...
      const updates = await updateEstimation.reload();
      return onSuccess(res, 200, "subcontractor Updated successfully", updates);
    } catch (error) {
      return onError(
        res,
        500,
  ...
  static async getAllProjectEstimation(req, res){
    try{
      ...
      return onSuccess(res, 200, 'successfully returned', estimations)
